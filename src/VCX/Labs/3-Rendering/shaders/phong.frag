...
vec3 Shade(vec3 lightIntensity, vec3 lightDir, vec3 normal, vec3 viewDir, vec3 diffuseColor, vec3 specularColor, float shininess) {
    ...
    if (u_UseBlinn) {
        return diffuseColor * lightIntensity * max(0.0f, dot(lightDir, normal)) + specularColor * lightIntensity * pow(max(0.0f, dot(normal, normalize(lightDir + viewDir))), shininess);
    }

    return diffuseColor * lightIntensity * max(0.0f, dot(lightDir, normal)) + specularColor * lightIntensity * pow(max(0.0f, dot(2 * dot(lightDir, normal) * normal - lightDir, viewDir)), shininess);
...
vec3 GetNormal() {
    ...
    float Hll = texture(u_HeightMap, v_TexCoord).x;    //-- height from bump map texture, v_TexCoord=texture coordinates
    ...
    bumpNormal = normalize(abs(det) * vn - surf_grad);  // bump normal
